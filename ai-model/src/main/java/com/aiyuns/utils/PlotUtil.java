...
public class PlotUtil {
  ...
  public static void plotTrainingData(
      MultiLayerNetwork model, DataSetIterator trainIter, INDArray backgroundIn, int nDivisions) {
    ...
    JPanel panel =
        new ChartPanel(
            createChart(
                ...
                maxs,
                nDivisions,
                createDataSetTrain(ds.getFeatures(), ds.getLabels())));
  ...
  public static void plotTestData(
      MultiLayerNetwork model, DataSetIterator testIter, INDArray backgroundIn, int nDivisions) {
    ...
    JPanel panel =
        new ChartPanel(
            createChart(
                ...
                maxs,
                nDivisions,
                createDataSetTest(ds.getFeatures(), ds.getLabels(), predicted)));
  ...
  private static XYDataset createDataSetTrain(INDArray features, INDArray labels) {
    ...
    INDArray argMax =
        Nd4j.getExecutioner().exec(new ArgMax(new INDArray[] {labels}, false, new int[] {1}))[0];
    for (int i = 0; i < nRows; i++) {
  ...
  public static INDArray generatePointsOnGraph(
      double xMin, double xMax, double yMin, double yMax, int nPointsPerAxis) {
  ...
  /**
   * This is to collect all the data and return it as one minibatch. Obviously only for use here
   * with small datasets
   *
   * @param iter
   * @return
