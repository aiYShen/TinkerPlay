...
/**
 * Given a base url and a zipped file name downloads contents to a specified directory under
 * ~/dl4j-examples-data Will check md5 sum of downloaded file
 *
 * <p>Sample Usage with an instantiation
 * DATAEXAMPLE(baseurl,"DataExamples.zip","data-dir",md5,size):
 *
 * <p>DATAEXAMPLE.Download() & DATAEXAMPLE.Download(true) Will download DataExamples.zip from
 * baseurl/DataExamples.zip to a temp directory, Unzip it to ~/dl4j-example-data/data-dir Return the
 * string "~/dl4j-example-data/data-dir/DataExamples"
 *
 * <p>DATAEXAMPLE.Download(false) will perform the same download and unzip as above But returns the
 * string "~/dl4j-example-data/data-dir" instead
 *
 * @author susaneraly
 */
public enum DownloaderUtility {
  ...
  ANOMALYSEQUENCEDATA(
      "anomalysequencedata.zip", "dl4j-examples", "51bb7c50e265edec3a241a2d7cce0e73", "3MB"),
  CAPTCHAIMAGE("captchaImage.zip", "dl4j-examples", "1d159c9587fdbb1cbfd66f0d62380e61", "42MB"),
  CLASSIFICATIONDATA(
      "classification.zip", "dl4j-examples", "dba31e5838fe15993579edbf1c60c355", "77KB"),
  ...
  PREDICTGENDERDATA(
      "PredictGender.zip", "dl4j-examples", "42a3fec42afa798217e0b8687667257e", "3MB"),
  ...
  private static final String AZURE_BLOB_URL =
      "https://dl4jdata.blob.core.windows.net/dl4j-examples";
   ...
   */
  DownloaderUtility(
      String baseURL, String zipFile, String dataFolder, String md5, String dataSize) {
    BASE_URL = baseURL;
  ...
  public String Download(boolean returnSubFolder) throws Exception {
    ...
    String extractDir =
        FilenameUtils.concat(System.getProperty("user.home"), "dl4j-examples-data/" + DATA_FOLDER);
    if (!new File(extractDir).exists()) new File(extractDir).mkdirs();
