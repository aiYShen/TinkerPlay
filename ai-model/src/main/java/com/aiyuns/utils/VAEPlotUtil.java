...
public class VAEPlotUtil {
  ...
  public static void scatterPlot(
      List<Pair<INDArray, INDArray>> data, List<Integer> epochCounts, String title) {
    ...
    JPanel panel =
        new ChartPanel(
            createChart(
                data.get(0).getFirst(),
                data.get(0).getSecond(),
                plotMin,
                plotMax,
                title + " (epoch " + epochCounts.get(0) + ")"));
        ...
        new ChangeListener() {
          ...
          public void stateChanged(ChangeEvent e) {
            ...
            JPanel panel =
                new ChartPanel(
                    createChart(
                        data.get(value).getFirst(),
                        data.get(value).getSecond(),
                        plotMin,
                        plotMax,
                        title + " (epoch " + epochCounts.get(value) + ")"));
            if (lastPanel != null) {
              f.remove(lastPanel);
            }
            lastPanel = panel;
  ...
  public static void plotData(
      List<INDArray> xyVsIter, INDArray labels, double axisMin, double axisMax, int plotFrequency) {
    ...
    slider.addChangeListener(
        new ChangeListener() {
          ...
          public void stateChanged(ChangeEvent e) {
            ...
            int value = slider.getValue();
            JPanel panel =
                new ChartPanel(createChart(xyVsIter.get(value), labels, axisMin, axisMax));
            if (lastPanel != null) {
              f.remove(lastPanel);
            }
            lastPanel = panel;
  ...
  private static String getTitle(int recordNumber, int plotFrequency) {
    return "MNIST Test Set - Latent Space Encoding at Training Iteration "
        + recordNumber * plotFrequency;
  }
  ...
  private static JFreeChart createChart(
      INDArray features, INDArray labels, double axisMin, double axisMax) {
    return createChart(
        features,
        labels,
        axisMin,
        axisMax,
        "Variational Autoencoder Latent Space - MNIST Test Set");
  }

  private static JFreeChart createChart(
      INDArray features, INDArray labels, double axisMin, double axisMax, String title) {
  ...
  public static class MNISTLatentSpaceVisualizer {
    ...
    private String getTitle(int recordNumber) {
      return "Reconstructions Over Latent Space at Training Iteration "
          + recordNumber * plotFrequency;
    }
    ...
    private List<JLabel> getComponents(int idx) {
      ...
      for (INDArray arr : temp) {
        ...
        Image imageScaled =
            orig.getImage()
                .getScaledInstance(
                    (int) (imageScale * 28), (int) (imageScale * 28), Image.SCALE_REPLICATE);
