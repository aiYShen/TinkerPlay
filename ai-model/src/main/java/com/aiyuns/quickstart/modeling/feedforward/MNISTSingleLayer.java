 ...
 *
 •	输入层：该层的输入维度为 numRows*numColumns，其中这些变量表示图像中垂直和水平像素的数量。
 此层使用 ReLU（修正线性单元）激活函数。该层的权重使用 Xavier初始化（了解Xavier初始化）进行初始化，以避免学习曲线过陡。
 此层将有1000个输出信号传递给隐藏层。
 •	隐藏层：该层的输入维度为1000，这些信号来自输入层。该层的权重同样使用 Xavier初始化。
 此层的激活函数为 Softmax，它对所有10个输出进行标准化，使得标准化后的和为1。
...
public class MNISTSingleLayer {
    ...
    public static void main(String[] args) throws Exception {
        ...
        MultiLayerConfiguration conf = new NeuralNetConfiguration.Builder()
                ...
                .l2(1e-4)
                .list()
                .layer(new DenseLayer.Builder() //创建第一个输入层并使用 Xavier 初始化
