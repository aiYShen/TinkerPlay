...
/**
 * @Author: aiYunS @Date: 2022-10-11 下午 03:10 @Description: 迪杰斯特拉算法:寻找从一个顶点到其余各顶点的最短路径算法，可用来解决最短路径问题
 * * 案例：最短路径问题 * 1. 战争时期，胜利乡有7个村庄(A,B,C,D,E,F,G)，现在有6个邮差，从G点出发，需要分别把邮件分别送到A,B,C,D,E,F 六个村庄 * 2.
 * 各个村庄的距离用边线表示（权），比如A-B距离5公里 * 3. 问：如何计算最短距离
 */

/**
 * @desc 迪杰斯特拉算法 案例：最短路径问题 1. 战争时期，胜利乡有7个村庄(A,B,C,D,E,F,G)，现在有6个邮差，从G点出发，需要分别把邮件分别送到A,B,C,D,E,F 六个村庄
 *     2. 各个村庄的距离用边线表示（权），比如A-B距离5公里 3. 问：如何计算最短距离 @Author xw @Date 2019/10/8
 */
