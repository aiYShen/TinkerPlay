...
/**
 * @Author: aiYunS @Date: 2022-8-11 下午 04:10 @Description:
 */
public class JdbcPool implements IPool {
  ...
  /** 通过实例初始化块来初始化 */
  {
    ...
    InputStream is = JdbcPool.class.getClassLoader().getResourceAsStream("db2.properties");
    ...
    initConnectCount = Integer.valueOf(properties.getProperty("db.initConnectCount"));
    maxConnects = Integer.valueOf(properties.getProperty("db.maxConnects"));
    incrementCount = Integer.valueOf(properties.getProperty("db.incrementCount"));
  ...
  private synchronized PoolConnection getActiveConnection() {
    ...
    while (true) {
      ...
      } else if (connections.size() < maxConnects
          && connections.size() > maxConnects - incrementCount) {
