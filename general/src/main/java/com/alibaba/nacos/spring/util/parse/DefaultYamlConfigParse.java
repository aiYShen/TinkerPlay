...
/**
 * @Author: aiYunS @Date: 2023年12月19日, 0019 下午 2:10:50 @Description: 解决由于升级springboot 3.2.0版本导致
 * org.yaml.snakeyaml 2.2版本的SafeConstructor类移除了无参构造方法 替代类
 */
public class DefaultYamlConfigParse extends AbstractConfigParse {
  ...
  protected static boolean process(
      DefaultYamlConfigParse.MatchCallback callback, Yaml yaml, String content) {
    int count = 0;
  ...
  protected static boolean process(
      Map<String, Object> map, DefaultYamlConfigParse.MatchCallback callback) {
    if (LOGGER.isDebugEnabled()) {
  ...
  protected static void buildFlattenedMap(
  ...
  public Map<String, Object> parse(String configText) {
    ...
    process(
        new DefaultYamlConfigParse.MatchCallback() {
          @Override
